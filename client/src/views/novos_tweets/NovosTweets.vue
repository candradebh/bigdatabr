<template>
  <div class="container">
      ...
      <div class="col-lg-4">
        <div class="card text-white bg-info mb-3" style="max-width: 18rem;">
            ...
            <div class="card-body">
              ...
              <p class="card-text">Você avaliou {{avaliados}} tweets de {{ total }} tweets no total</p>
            </div>
          </div>
      </div>
    </div>
    ...
    <div class="row" v-if="tweets">
      ...
      <table class="table table-hover table-bordered table-striped">
        ...
        <tbody>
          <tr v-for="tweet in tweets" v-bind:key="tweet.id">
            ...
            <td>
              <div class="btn-group">
                <button class="btn btn-sm btn-secondary" v-bind:class="{ 'btn-danger': tweet.avaliacao != '' && tweet.avaliacao != tweet.sentimento }" @click="avaliar(tweet,'errou')">Errou</button>
                <button class="btn btn-sm btn-secondary" v-bind:class="{ 'btn-success': tweet.avaliacao != '' && tweet.avaliacao == tweet.sentimento }" @click="avaliar(tweet,'acertou')">Acertou</button>
              </div>
            </td>
      ...
      v-if="avaliados > 0" >Enviar Avaliação</button>
    </div>
  </div>
</template>
